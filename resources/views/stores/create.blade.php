                    ...
                    <form method="POST" action="{{ route('stores.store') }}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <textarea class="form-control @error('opening_hours') is-invalid @enderror"
                                          ...
                                          name="opening_hours"
                                          rows="3"
                                          placeholder="平日: 9:00-18:00&#10;土日: 10:00-17:00">{{ old('opening_hours') }}</textarea>
                                @error('opening_hours')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror"
                                    ...
                                    <option value="preparing" {{ old('status') == 'preparing' ? 'selected' : '' }}>準備中</option>
                                    <option value="open" {{ old('status') == 'open' ? 'selected' : '' }}>営業中</option>
                                    <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>休業中</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror"
                                      ...
                                      name="description"
                                      rows="4"
                                      placeholder="店舗の特徴や備考事項を入力してください">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
